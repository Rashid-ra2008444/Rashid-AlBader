...
fun ProductScreen(
    ...
    QuickMartTheme {
        val category by productViewModel.categories.collectAsState()
        Scaffold(topBar = {
            TopBar(
                query = productViewModel.query,
                categories = category,
                searchBy = { query -> productViewModel.updateList(query) },
                filterBy = { category -> productViewModel.updateList(query = productViewModel.query, categoryId = category.id) })
...
//    onAddProductToCart :(Product) -> Unit,
    ...
    LazyVerticalStaggeredGrid(columns = StaggeredGridCells.Fixed(2), modifier = modifier) {
        itemsIndexed(products) { i, product ->
            ...
            val categoryName =
                category.find { it.id == product.categoryId }?.category ?: "Unknown"
            ProductCard(
                product = product,
                quantity = cartViewModel.getQuantity(product.id),
                isFavourite = favouriteViewModel.isInFavourite(userId , product.id),
                onAddProductToCart = {
                ...
                onProductSelected = {
                    productViewModel.selectedProduct = products.getOrNull(i) ?: return@ProductCard
                    onProductSelected()
                },
                onToggleFavourite = {
                    Toast.makeText(context, "Hi", Toast.LENGTH_SHORT).show()
                    if (favouriteViewModel.isInFavourite(userId = userId, productId = product.id))
                        favouriteViewModel.onRemoveProductFromFavourite(productId= product.id,userId =userId)
                    else
                        favouriteViewModel.addToFavourite(productId = product.id, userId = userId)
