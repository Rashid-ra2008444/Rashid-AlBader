...
import com.example.quickmart.ui.user.UserViewModel
...
fun AppNavHost(navController: NavHostController, paddingValues: PaddingValues) {
    ...
    NavHost(navController, NavDestinations.Shop.route, modifier = Modifier.padding(paddingValues)) {
        ...
        composable("Cart", enterTransition = { slideInHorizontally { it / 2 } }) {
            CartScreen(cartViewModel = cartViewModel, productViewModel = productViewModel , onCheckOut = ({ Unit }) , userId) {
        ...
        composable(
            ...
            FavouriteScreen(favouriteViewModel , userId)
        }
        composable(
