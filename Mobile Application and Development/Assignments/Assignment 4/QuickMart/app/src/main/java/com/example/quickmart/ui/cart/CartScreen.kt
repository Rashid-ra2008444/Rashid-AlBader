...
fun CartScreen(
    ...
    val productIds = cartItems.value.map { it.productId}
    val cartProducts by productViewModel.cartProducts.collectAsState()

    LaunchedEffect(key1 = cartItems.value ) {
        Log.d("CartScreen", "Product IDs in cart: ${cartItems.value.map { it.productId }}")
        productViewModel.getMatchingProducts(cartItems.value.map { it.productId })
    }
    LaunchedEffect(key1 = userId ){
        Log.d("CartScreen", "User ID: $userId")
            ...
            content = {
                ...
                CartItemsList(
                    ...
                    onDecreaseQuantity = { cartItem ->
                        cartViewModel.addOrUpdate(cartItem.productId, -1, userId)
    ...
    if (cartItems.value.isEmpty())
        ...
        LazyColumn(modifier = modifier.padding(bottom = 80.dp)) {
            items(cartItems.value) { cartItem ->
                val cartProduct = cartProducts.find { it.id == cartItem.productId }
