...
fun FavouriteScreen(
    ...
    val favouriteProducts by favouriteViewModel.matchingProducts.collectAsState()
    val favourites by favouriteViewModel.favourites.collectAsState()

    LaunchedEffect(key1 = favourites) {
        favouriteViewModel.getMatchingProducts(favourites.map { it.productId })
    }
        ...
        Scaffold(
            ...
            floatingActionButton = {
                if (favouriteProducts.isNotEmpty()) {
                    ElevatedButton(
                        onClick = {
                            favouriteViewModel.addAllFavToCart(userId)
                            Toast.makeText(context, "Added all favourites to cart", Toast.LENGTH_SHORT).show()
                        },
                        colors = ButtonDefaults.buttonColors(
                            containerColor = Color.hsl(150F, 0.73F, 0.38F)
                        ),
